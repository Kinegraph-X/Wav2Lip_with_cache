...
import numpy as np
import scipy, cv2, os, sys, argparse, libs.audio as audio
...
def main():
	...
	# elif args.face.split('.')[1] in ['jpg', 'png', 'jpeg']:
		...
		end_time = time.perf_counter()
		print (f'Effecrive VideoCapure took : {end_time - start_time}')
	...
	wav = audio.load_wav(hparams.test_audio_file_path, 16000)
	# wav = audio.load_audio(hparams.test_audio_file_path, 16000)
	# wav = audio.torch_load_audio(hparams.test_audio_file_path, 16000)
	end_time = time.perf_counter()
	...
	# command = '{} ffmpeg -y -i {} -i {} -strict -2 -q:v 1 {}'.format(hparams.ffmpeg_path, args.audio, 'temp/result.avi', args.outfile)
	command = '{}ffmpeg -y -i {} -i {} -strict -2 -q:v 1 {}'.format(hparams.ffmpeg_path, hparams.test_audio_file_path, 'temp/result.avi', args.outfile)
